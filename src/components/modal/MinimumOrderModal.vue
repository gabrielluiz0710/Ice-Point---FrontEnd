        ...
        <div v-if="show" class="modal-overlay" @mousedown.self="emit('close')">
            <div class="modal-content" role="dialog">
                ...
                <div class="modal-body">
                    <h2 id="modal-title">Pedido Mínimo</h2>
                    <p>
                        Você precisa de pelo menos 80 picolés no carrinho para finalizar a compra.
                    </p>
                    <p v-if="remaining > 0" class="remaining-items">
                        Falta{{ remaining === 1 ? '' : 'm' }} apenas {{ remaining }} picolé{{ remaining
                            === 1 ? '' : 's' }}!
                    </p>
                </div>
...
.modal-icon-info {
    ...
    height: 100px;
    border-radius: 50%;
    background: linear-gradient(145deg, var(--c-azul), var(--c-azul-dark));
